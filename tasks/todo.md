...
| FS-6 | P1 | `/edit_instructor/<id>` renders `edit_instructor.html`, which **does not exist** (file is `edit_instructors.html`). Route returns 500. | [app.py:1047](../app.py#L1047) |
...
| ID | Sev | Finding | Evidence |
|---|---|---|---|
| SE-1 | P0 | **Passwords stored and compared in plaintext** for both admin and instructors; schema seeds `admin`/`admin`. | [app.py:73-99](../app.py#L73-L99), [schema.sql:61](../database/schema.sql#L61) |
| SE-2 | P0 | **`/video_feed` requires no authentication** — anyone who can reach the host gets the live camera stream of a classroom. | [app.py:1456](../app.py#L1456) |
| SE-3 | P0 | **Path traversal → arbitrary directory deletion.** `student_id`/`name` come from an unvalidated form, are stored, then concatenated into a path passed to `shutil.rmtree()` and `os.rename()`. A student named `..\..\Windows\Temp` gives an authenticated user directory deletion outside `dataset/`. | [app.py:320-328](../app.py#L320-L328), [app.py:506-535](../app.py#L506-L535), [app.py:666-681](../app.py#L666-L681) |
| SE-4 | P1 | **Missing auth checks** on `/edit_instructor`, `/update_instructor`, `/delete_instructor`, `/export_excel`, `/start-attendance`, `/stop_camera`, `/video_feed` — unauthenticated instructor CRUD and camera control. | [app.py:1030,1053,1083,1114,1151,1332,1456](../app.py#L1030) |
| SE-5 | P1 | **No role authorization.** Every protected route checks only `'user' in session`. A logged-in *instructor* can call `/delete_student`, `/manage_instructors`, `/update_admin`, `/change_password`. The sidebar hides links; the routes do not enforce. | [app.py](../app.py) app-wide |
| SE-6 | P1 | **Destructive operations exposed over GET**: `/delete_subject/<id>`, `/delete_instructor/<id>`. Triggerable by an `<img src>` or a link prefetch. | [app.py:879,1083](../app.py#L879) |
| SE-7 | P1 | **No CSRF protection** on any of the ~15 state-changing POST routes. No Flask-WTF, no token. | app-wide |
...
| SE-9 | P1 | **Biometric data governance absent.** Face images stored unencrypted on disk, no consent record, no retention/erasure policy, no access log. Directly relevant to RA 10173 (Philippine Data Privacy Act) — sensitive personal information. An examiner will ask. | `dataset/` |
| SE-10 | P2 | Raw database errors returned to the client (`f"Database error: {error}"`) — schema/version disclosure. | [app.py:365,430,601,1253](../app.py#L365) |
| SE-11 | P2 | No session hardening: no `SESSION_COOKIE_SECURE`, no `SAMESITE`, no `PERMANENT_SESSION_LIFETIME`, no idle timeout. | [app.py:20-21](../app.py#L20-L21) |
...
| SE-13 | P2 | No rate limiting or lockout on `/login` — unbounded credential brute force. | [app.py:60](../app.py#L60) |
| SE-14 | P3 | `admin_panel.py` executes `admin_login()` **at import time**, prompting for credentials on any import. | [admin_panel.py:385](../admin_panel.py#L385) |
| **SE-15** | **P1** | **Passwords are compared case-insensitively, and trailing whitespace is ignored.** The `password` columns use `utf8mb4_general_ci` — a case-insensitive, PAD SPACE collation — and the app compares passwords *in SQL* rather than in Python. **Measured against the live database:** password `admin` is accepted as `ADMIN`, `AdMiN`, and `admin   `. Usernames behave the same way. This collapses an alphanumeric alphabet from 62 symbols to 36: an 8-character password drops from ~2.2×10¹⁴ to ~2.8×10¹² combinations, ~79× weaker, on top of already being stored in plaintext (SE-1). Fixed for free by the Phase 2 bcrypt migration, since bcrypt compares hashes in Python and never lets collation decide authentication. | [app.py:73-78](../app.py#L73-L78), [app.py:94-99](../app.py#L94-L99); verified 2026-08-08 |
...
- [ ] `@login_required` + `@role_required('admin')` decorators; **apply to every route**, audit the full route table (SE-4, SE-5)
- [ ] Convert all destructive routes to POST (SE-6)
- [ ] Flask-WTF CSRF on all forms (SE-7)
- [ ] **Path-traversal fix**: validate `student_id`/`name` against a strict allowlist regex at input; derive dataset folder from a **sanitised slug or surrogate key**, never raw user input; assert the resolved path is inside `dataset/` before any `rmtree`/`rename` (SE-3)
- [ ] Protect `/video_feed` and all camera-control routes (SE-2)
- [ ] Generic error pages; log details server-side only (SE-10, US-1)
- [ ] Session hardening: `SECRET_KEY` from env, `HTTPONLY`, `SAMESITE=Lax`, `PERMANENT_SESSION_LIFETIME` (SE-11)
- [ ] Login rate limiting / lockout (SE-13)
- [ ] Write `docs/data_privacy.md`: consent flow, retention period, erasure procedure, access log, RA 10173 mapping (SE-9)
- [ ] **Verify:** security test suite — each fixed vuln gets a regression test
